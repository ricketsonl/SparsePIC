...
def idatgen(N,Lx,Ly,MAX):
    ...
    for i in range(0,N):
        xxe[i] = np.random.uniform(0.,Lx,1)
        xxi[i] = np.random.uniform(0.,Lx,1)
#        vxe[i] = np.random.uniform(-0.5*Lx,0.5*Lx,1)
#        vxi[i] = np.random.uniform(-0.5*Lx,0.5*Lx,1)
        vxe[i] = np.random.normal(0.,1.,1)
        vxi[i] = np.random.normal(0.,1.,1)
        xye[i] = np.random.uniform(0.,Ly,1)
        xyi[i] = np.random.uniform(0.,Ly,1)
#        vye[i] = np.random.uniform(-0.5*Ly,0.5*Ly,1)
#        vyi[i] = np.random.uniform(-0.5*Ly,0.5*Ly,1)
        vye[i] = np.random.normal(0.,1.,1)
        vyi[i] = np.random.normal(0.,1.,1)
        f0e[i] = 1./(Lx*Ly*2*np.pi)*np.exp(-0.5*vxe[i]**2-0.5*vye[i]**2)
        f0i[i] = 1./(Lx*Ly*2*np.pi)*np.exp(-0.5*vxi[i]**2-0.5*vyi[i]**2)
